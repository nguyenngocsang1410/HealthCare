...
//import com.fanny.healthcare.activity.ZNSBActivity;
		...
		synchronized (this) {
			while (!stop) {
				try {
					...
					if (ZNSBActivity2.SPO_RECT.size() > 0) {
						Wave data = ZNSBActivity2.SPO_RECT.remove(0);
						if(data!=null){
							this.spo = data.data;
						}
						postInvalidate();
						if (ZNSBActivity2.SPO_RECT.size() > 25) {
							Thread.sleep(17);
						} else {
							Thread.sleep(20);
						}
